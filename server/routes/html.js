...
export default () => {
  ...
  <head>
    ...
    <% if (assets.version) { %><link rel="stylesheet" type="text/css" href="<%= assets.cdnPath %>/auth0-visualstudio-deploy.ui.<%= assets.version %>.css"><% } %>
  </head>
  <body class="a0-extension">
    ...
    <% if (assets.version) { %>
    <script type="text/javascript" src="<%= assets.cdnPath %>/auth0-visualstudio-deploy.ui.vendors.<%= assets.version %>.js"></script>
    <script type="text/javascript" src="<%= assets.cdnPath %>/auth0-visualstudio-deploy.ui.<%= assets.version %>.js"></script>
    <% } %>
  </body>
  </html>
  `;
  ...
  return (req, res) => {
    ...
    if (clientVersion) {
      const cdnPath = config('CDN_PATH') || (
        PR_NUMBER
          ? `//s3.amazonaws.com/extensions-review/auth0-visualstudio-deploy-pr-${PR_NUMBER}/assets`
          : '//cdn.auth0.com/extensions/auth0-visualstudio-deploy/assets'
      );
